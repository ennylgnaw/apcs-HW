...
/*======================================
  ...
  n=5   15.2406s    over 5 executions 
  n=6   4.8751m    over 2 executions
  n=7   NAs    over __ executions
  n=8   NAs    over __ executions
  ======================================*/
...
class TourFinder 
{
    ...
    //helper to check number of possible moves
    public int checkSpots (int x, int y) {
	int poss = 0;
	int[] xVals = {-1,-1,1,1,-2,-2,2,2};
	int[] yVals = {-2,2,-2,2,-1,1,-1,1};
	for (int i = 0; i < xVals.length; i++) {
	    try {
		if (board[x + xVals[i]][y + yVals[i]] == 0)
		    poss += 1;
	    }
	    catch (Exception e) {}
	}
	...
	//and recursively generate tour possibilities from current pos
	    ...
	    for (int i = 0; i < xVals.length; i++) {
		try {
		    if (checkSpots(x + xVals[i], y + yVals[i]) < smallest) {
			smallest = checkSpots(x + xVals[i], y + yVals[i]);
			xVal = x + xVals[i]; yVal = y + yVals[i];
		    }
	        }
		catch (Exception e) {}
	    }
