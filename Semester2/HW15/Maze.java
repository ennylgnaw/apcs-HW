 ...
 * 1. Check if current space is exit - stop recursing if yes, go to step 2 
      if not
   2. Check if current space is a wall or visited path if yes, go to step 4, 
      if not, go to step 3
   3. Move the hero to current space, go to step 4
   4. Restart the algo from step 1 with the next available space in relation 
      to the hero(up, down, left, or right)
 ***/
